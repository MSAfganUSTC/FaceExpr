...
class AttentionFocusedFusion(nn.Module):
    def __init__(self, unet_encoder, unet_decoder, r_net, input_dim=512):
        super().__init__()
        self.temporal_emb = TemporalEmbedding(input_dim)
        self.unet_encoder = unet_encoder
    ...
    def forward(self, I_id, P_id, P_exp, T_steps, f_exp_n):
        T_emb = self.temporal_emb(T_steps)
        ...
        for t in range(T_steps, 0, -1):
            ...
            for i in range(1, 5):
                if i in [2, 3] and (0.3 * T_steps <= t <= 0.7 * T_steps):
                    f_id_n = self.unet_encoder(self.r_net(self.p_atten(I_id), P_id + P_exp), T_emb_t)
                
                X = self.unet_encoder(self.r_net(self.c_atten(I_id), P_id), T_emb_t)
            
            if i in [2, 3] and (0.3 * T_steps <= t <= 0.7 * T_steps):
                f_fus_n = F.softmax(torch.bmm(f_exp_n, f_id_n.transpose(1, 2)) / (input_dim ** 0.5), dim=-1) @ f_id_n \
                          + F.softmax(torch.bmm(f_id_n, f_exp_n.transpose(1, 2)) / (input_dim ** 0.5), dim=-1) @ f_exp_n
                X_skip = self.r_net(X, f_fus_n)
            else:
                X_skip = self.r_net(X)
            
            Y = self.unet_decoder(self.r_net(self.c_atten(X_skip), P_id), T_emb_t)
...
if __name__ == "__main__":
    ...
    extractor = ExpressionFeatureExtractor(unet_encoder, unet_decoder, r_net)
    I_ref, f_exp_n, fused_features = extractor(input_image, P_exp, T_steps)
    
    fusion = AttentionFocusedFusion(unet_encoder, unet_decoder, r_net)
    I_gen = fusion(input_image, P_id, P_exp, T_steps, f_exp_n)
    
    print(f"Generated image shape: {I_gen.shape}")
